...
/*
하나의 중위 표기식을 입력하며 프로그램은 이 수식을 계산하여 결과를 출력한다. 강의 슬라이드에 제시
한 프로그램과 달리 음수 피연산자나 실수 피연산자를 허용해야 하며 계산의 결과도 실수여야 한다. 또
한 사용자가 수식을 입력할 때 토큰들 사이에 한 칸씩 띄운다는 제약도 없다. 즉, 사용자는 연산자와 피연
산자를 붙여서 입력할 수도 있고, 두 칸 이상  띄워서 입력할 수도 있다. 프로그램에서 후위표기식으로 변
환하는 단계와 후위표기식을 계산하는 단계를 통합하여 변환과 동시에 계산이 이루어지도록 구현하라.


음수 피연산자나 실수 피연산자를 허용해야 하며 계산의 결과도 실수여야 한다
사용자는 연산자와 피연산자를 붙여서 입력할 수도 있고, 두 칸 이상  띄워서 입력할 수도 있다
프로그램에서 후위표기식으로 변환하는 단계와 후위표기식을 계산하는 단계를 통합하여 변환과 동시에 계산이 이루어지도록 구현하라.


1. 사용자가 띄워서 입력하는 경우, 공백을 무시하며 스택에 집어넣음
2. "-"의 경우
    1. 앞에 피연산자가 있을 때 -> 그냥 뻴셈
    2. 없을 때 -> 음수처리
*/

// 1. 사용자가 띄워서 입력하는 경우, 공백을 다 없애기
